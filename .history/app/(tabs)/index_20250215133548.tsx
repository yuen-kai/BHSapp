...
export default function HomeScreen() {
	...
	const weekDaySchedule = [
		...
		[//sat temp, had to add so app still works, 
			//{ block: "Saturday", start: "12:00 am", end: "11:59 pm", lunch: false }
			{ block: "A", start: "8:20 am", end: "9:15 am", lunch: false },
			{ block: "B", start: "9:22 am", end: "10:17 am", lunch: false },
			{ block: "C", start: "10:24 am", end: "11:19 am", lunch: false },
			{ block: "D", start: "11:26 am", end: "12:51 pm", lunch: false },
			{ block: "F", start: "12:51 pm", end: "1:15 pm", lunch: true },
			{ block: "F", start: "1:00 pm", end: "1:53 pm", lunch: true },
			{ block: "G", start: "2:00 pm", end: "2:55 pm", lunch: false }
		]
	];
	...
	const findCurrentBlock = (): Schedule => {
		...
		for (let i = 0; i < schedule.length; i++) {
			...
			if (startTime < now && now < endTime && schedule[i].lunch == true) {
				return (findCurrentClass(findCurrentBlock().block).lunch == 1) ? schedule[i+1] : schedule[i]
			}
	...
	const findCurrentClass = (block: string): Course => {
	...
	const currentClass = findCurrentClass(currentBlock.block);
	...
	return (
		...
		>
			...
			<Card style={styles.courseCard}>
				<Card.Content>
					<Text style={styles.courseInfoTitle}>{currentClass.name || 'No Class'}</Text>
					<Text style={styles.courseInfoSubTitle}>
						{currentClass?.teacher || 'No Teacher'} • Room {currentClass?.roomNumber || 'N/A'}
					</Text>
					<Text style={styles.courseInfoSubTitle}>
