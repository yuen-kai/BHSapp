...
export default function Account({ session }: { session: Session }) {
	...
	async function updateProfile({
		...
		try {
			...
			//prints out updates with supposed data, but outputs stuff in alphabetical order
			console.log(updates)
			const { error } = await supabase.from("profiles").upsert(updates, { onConflict: 'id' }).select();
